...
#
# The timeout is below CI's job bound, so a hung test is ended by Go —
# which prints every goroutine's stack — rather than by the runner, which
# prints nothing.
test-race:
    go test -race -timeout 5m ./...
    cd docker && go test -race -timeout 5m ./...
