...
//
// It creates a shallow copy of cmd and overrides Stdout/Stderr with internal buffers.
// The caller's original Command is not mutated. If you need simultaneous streaming
// and buffering, compose with io.MultiWriter on the Command before calling Run directly.
...
//
// It creates a shallow copy of cmd and overrides Stdout with an internal pipe.
// The caller's original Command is not mutated. Useful for live logging of build
// output or deployment progress.
...
// duration of the command. The original terminal state is restored via defer,
// ensuring cleanup even if the command panics.
