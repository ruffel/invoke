...
func getTestPaths(t T, _ invoke.Environment) (string, string) {
...
func joinRemote(_ invoke.Environment, base string, parts ...string) string {
...
func fileContracts() []TestCase {
	return []TestCase{
		...
		{
			Category:    CategoryFilesystem,
			Name:        "download-failure-source-missing",
			Description: "Error returned when we try to download a non-existent remote file",
			Run: func(t T, env invoke.Environment) {
				remoteBase, _ := getTestPaths(t, env)
				remotePath := joinRemote(env, remoteBase, "this-remote-file-really-does-not-exist-99999")
				localPath := filepath.Join(t.TempDir(), "should-not-exist.txt")

				err := env.Download(t.Context(), remotePath, localPath)
				require.Error(t, err)
			},
		},
		{
			...
			Run: func(t T, env invoke.Environment) {
				...
				res, err := exec.RunBuffered(t.Context(), invoke.ShellCommand(readCmd+" "+dstPath))
				require.NoError(t, err)
		...
		},
		{
			...
			Run: func(t T, env invoke.Environment) {
				...
				exec := invoke.NewExecutor(env)
				// Check file 1
				res, err := exec.RunBuffered(t.Context(), invoke.ShellCommand(readCmd+" "+joinRemote(env, dstDir, "file1.txt")))
				require.NoError(t, err)
				require.Equal(t, "root file", strings.TrimSpace(string(res.Stdout)))

				// Check file 2 (nested)
				res, err = exec.RunBuffered(t.Context(), invoke.ShellCommand(readCmd+" "+joinRemote(env, dstDir, "subdir", "file2.txt")))
				require.NoError(t, err)
		...
		},
		{
			...
			Prereq: func(_ T, _ invoke.Environment) (bool, string) {
				return true, ""
			},
			Run: func(t T, env invoke.Environment) {
