...
func (e *Environment) Upload(ctx context.Context, localPath, remotePath string, opts ...invoke.FileOption) error {
	...
	// Normalize the remote path. We use the container root as the base
	...
	err := e.client.CopyToContainer(ctx, e.config.ContainerID, "/", reader, options)
	if err != nil {
...
func untar(r io.Reader, dst string, perms os.FileMode) error {
	...
	if firstHeader.Typeflag == tar.TypeReg {
		return extractFileFromTar(tr, firstHeader, dst, perms)
	}

	// Process first header
	if err := processTarHeader(tr, firstHeader, dst, perms); err != nil {
		return err
	}
	...
	for {
		...
		if err := processTarHeader(tr, header, dst, perms); err != nil {
			return err
		}
	}
...
func extractFileFromTar(tr *tar.Reader, header *tar.Header, dst string, perms os.FileMode) error {
...
func processTarHeader(tr *tar.Reader, header *tar.Header, dstRoot string, perms os.FileMode) error {
	if strings.Contains(header.Name, "..") {
		return fmt.Errorf("illegal file path in tar: %s", header.Name)
	}

	return extractEntry(dstRoot, header, tr, perms)
}
