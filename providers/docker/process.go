...
func (p *Process) Wait() (*invoke.Result, error) {
	...
	if p.closed {
		...
		return nil, errors.New("process closed")
	}
	...
	if p.result.Error != nil {
		return res, p.result.Error
	}

	if p.result.ExitCode != 0 {
		return res, &invoke.ExitError{
...
// Signal closes the hijacked connection to terminate the exec process.
// Docker does not support native POSIX signaling for exec processes.
