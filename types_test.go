...
func TestShellCommand(t *testing.T) {
	...
	assert.Equal(t, &Command{Cmd: "sh", Args: []string{"-c", "echo hello"}}, cmd)
}
...
func TestParseTargetOS(t *testing.T) {
	...
	tests := []struct {
		...
		{"windows returns unknown", "windows", OSUnknown},
	}
...
func TestTransportError(t *testing.T) {
	...
	t.Run("error with command", func(t *testing.T) {
		t.Parallel()
		...
		assert.Contains(t, e.Error(), "ls -la")
		assert.Contains(t, e.Error(), "connection reset")
	})

	t.Run("error without command", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("unwrap returns cause", func(t *testing.T) {
		...
		e := &TransportError{Err: cause}
		assert.ErrorIs(t, e, cause)
	})
}

func TestCommand_Validate_WhitespaceOnly(t *testing.T) {
	t.Parallel()
